...
from lumo.finger.fingertip_geometry import FingertipModel, PolygonalGeometry
...
class FingertipSolid:
    ...
    operation in :mod:`lumo.mesh.volume.mesh`.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.pad_geometry.intersection(self.rigid_geometry).area
            > self.parameters.geometry_area_tolerance_mm2
        ):
            raise ValueError("pad and rigid solid regions overlap")
        if self.material_geometry.symmetric_difference(
            self.pad_geometry
        ).area > self.parameters.geometry_area_tolerance_mm2:
