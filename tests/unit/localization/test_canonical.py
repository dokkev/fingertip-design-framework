...
def _region(*, slope: float = 0.0) -> FingertipBoundaryRegion:
...
def test_slanted_finger_texture_maps_to_same_canonical_coordinates() -> None:
    ...
    straight_warp = warp_to_canonical(
        ...
        build_canonical_finger_map(straight, output_height=32, output_width=24),
    )
    slanted_warp = warp_to_canonical(
        _finger_texture(slanted),
        build_canonical_finger_map(slanted, output_height=32, output_width=24),
    )
...
def test_similarity_transform_moves_reference_canonical_map() -> None:
    region = _region()
    reference_map = build_canonical_finger_map(region, output_height=8, output_width=6)
