...
def test_volume_state_direct_adapter_builds_full3d_geometry_without_fea_artifact() -> None:
    ...
    np.testing.assert_array_equal(
        geometry.rigid.faces,
        carrier_mesh.surface_mesh.faces[
            np.asarray(carrier_mesh.lateral_face_indices, dtype=np.int64)
        ],
    )
    expected_envelope_faces = 2 + sum(
        len(volume_mesh.surface_triangles[name])
        for name in (
            "support_bond_left",
