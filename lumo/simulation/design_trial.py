"""Prescribed force-threshold indentation trials for one fingertip design."""
...
class DesignStudy:
    """Run independent constant-speed trials and capture force crossings."""
    ...
    def _inspect_checkpoint(
        ...
        checkpoint = simulation.force_checkpoint(checkpoint_index)
        ...
        simulation.select_force_checkpoint(checkpoint_index)
        try:
    ...
    def _new_runtime(
        ...
        *,
    ...
    ) -> tuple[LumoSimulation, Indenter, newton.ModelBuilder | None]:
        if fingertip_model is None:
            ...
            return simulation, indenter, builder
        ...
        return simulation, indenter, None
    ...
    def _force_schedule(self, trial: DesignTrial) -> tuple[float, ...]:
        return self.force_targets_n or (trial.target_force_n,)
    ...
    def _run_parallel_graph(
        ...
        for trials in grouped_trials.values():
            ...
            for batch_start in range(0, len(trials), self.parallel_world_count):
                ...
                for trial in batch:
                    simulation, indenter, _ = self._new_runtime(
                        trial,
                        fingertip_model=fingertip_model,
                        indenter=indenter,
                    )
                    ...
                    if trial.initial_clearance_m is None:
                        raise ValueError(
                            "GPU force checkpoints require initial_clearance_m"
                        )
                    simulation.configure_force_checkpoints(
                        indenter,
                ...
                while not all(bool(world["finished"]) for world in worlds):
                    ...
                    for world in active:
                        ...
                        if remaining > 0:
                            simulation.launch_force_checkpoints(
                    ...
                    for world in active:
                        ...
                        checkpoint_count, finished, error = (
                            simulation.force_checkpoint_status(stream=world["stream"])
                        )
                        if error:
                            raise RuntimeError(
                        ...
                        if not finished and simulation.step_count >= int(
                            ...
                            last_force_n = simulation.current_reaction_force_n()
                            raise RuntimeError(
    ...
    def _run_graph_trial(
        ...
        *,
        ...
        force_targets_n = self._force_schedule(trial)
        if trial.initial_clearance_m is None:
            raise ValueError("GPU force checkpoints require initial_clearance_m")
        if reset_runtime:
            simulation.reset_force_checkpoints(indenter, initial_tf=trial.initial_tf)
        else:
            simulation.configure_force_checkpoints(
                indenter,
                initial_tf=trial.initial_tf,
        ...
        initial_translation, initial_rotation, motion_direction = self._trial_frame(
            trial
        )
        max_steps = int(trial.max_sim_time_s * self.sim_frequency)
        host_poll_ticks = 2 * max(1, ceil(0.1 * self.sim_frequency))
        processed = 0
        finished = False
        while simulation.step_count < max_steps and not finished:
            tick_count = min(host_poll_ticks, max_steps - simulation.step_count)
            checkpoint_count, finished, error = simulation.advance_force_checkpoints(
                tick_count
            )
            if error:
                raise RuntimeError(
                    f"{trial.name} GPU force checkpoints failed with code {error}"
                )
            while processed < checkpoint_count:
                self._inspect_checkpoint(
                    trial,
                    simulation,
                    indenter,
        ...
        if not finished:
            last_force_n = simulation.current_reaction_force_n()
            raise RuntimeError(
                f"{trial.name} did not cross {force_targets_n[processed]:g} N during "
                f"{trial.max_sim_time_s:g} s of simulation; last force was "
                f"{last_force_n:.9e} N"
            )

    def _run_direct_trial(
        self,
        trial: DesignTrial,
        simulation: LumoSimulation,
        indenter: Indenter,
        ...
        while simulation.step_count < max_steps:
            travel_m += trial.approach_speed_m_s * simulation.time_step_s
            pose = wp.transform(
                wp.vec3(*(initial_translation + travel_m * motion_direction)),
                initial_rotation,
            )
            ...
            trial.force_change_n = abs(signed_force_change_n)
            trial.force_threshold_n = target_force_n
            trial.force_overshoot_n = reaction_force_n - target_force_n
            trial.reaction_force_rate_n_s = (
                signed_force_change_n / simulation.time_step_s
            )
            trial.indentation_rate_m_s = trial.approach_speed_m_s
            if inspect_trial is not None:
                inspect_trial(trial, simulation, indenter)
            target_index += 1
        ...
        raise RuntimeError(
            f"{trial.name} did not cross {force_targets_n[target_index]:g} N during "
            f"{trial.max_sim_time_s:g} s of simulation; last force was "
            f"{previous_force_n:.9e} N"
        )
    ...
    def run(
        ...
        *,
        ...
        """Run and release trials, inspecting every force crossing."""
        if self._has_run:
        ...
        finalized_models: dict[Path, tuple[FingertipNewtonModel, Indenter]] = {}
        ...
        for trial in self.trials:
            ...
            if cached_runtime is not None:
                ...
                cached_model = finalized_models.get(model_key)
                simulation, indenter, builder = self._new_runtime(
                    trial,
                    fingertip_model=(None if cached_model is None else cached_model[0]),
                    indenter=(None if cached_model is None else cached_model[1]),
                )
                reset_runtime = False
                if self.reuse_finalized_models and cached_model is None:
            ...
            if self.use_cuda_graph:
                ...
                self._run_direct_trial(trial, simulation, indenter, inspect_trial)
            trial.wall_runtime_s = perf_counter() - start_s
            if not self.reuse_runtimes:
