...
def _make_carrier_collision_mesh(
    ...
    *,
    ...
    # lip and stem. Close the proxy through the carrier interior because
    ...
    if not cap_triangles or not isclose(
        ...
        raise ValueError(
            "carrier collision cap triangulation does not cover its boundary"
        )
    ...
    for triangle in cap_triangles:
        ...
        if len(coordinates) != 3:
            raise ValueError(
                "carrier collision cap triangulation produced a non-triangle"
            )
        if _signed_area(coordinates) < 0.0:
            coordinates = (
                coordinates[0],
    ...
    try:
        ...
        raise RuntimeError(
            "carrier collision meshing requires newton"
        ) from exc
