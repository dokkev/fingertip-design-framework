...
def compute_trajectory_objective(
    ...
    if zero_mass_indices:
        return {
            ...
            "objective_pathology_state_indices": zero_mass_indices,
    ...
    return {
        ...
        "worst_radius_pair": None if radius_pair is None else _pair_record(radius_pair[1], radius_pair[2], radius_pair[0]),
        "all_pairwise_distances": [float(value) for value in all_distances],
        "objective_pathology": bool(pathology),
        "objective_pathology_reason": (
            "near-total signal extinction"
            if pathology
            else None
        ),
        "objective_pathology_state_indices": extinct_states,
        "minimum_field_mass": float(min(field_masses)),
        "maximum_field_mass": float(max(field_masses)),
        "observation_count": len(items),
        "observation_diagnostics": diagnostics,
