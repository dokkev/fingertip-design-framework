...
def main() -> None:
    ...
    mechanics = build_fingertip_newton_model(mesh)
    model = mechanics.model

    if not model.body_flags.numpy()[mechanics.carrier_body] & int(
        newton.BodyFlags.KINEMATIC
    ):
        raise RuntimeError("carrier body is not kinematic")

    bonded_indices = mechanics.bonded_particle_indices.numpy()
    ...
    mechanics.prepare_step(state_0, state_1, identity)
    ...
    mechanics.prepare_step(state_0, state_1, translation)
    translated = state_0.particle_q.numpy()[bonded_indices]
    expected = mechanics.bonded_local_positions.numpy() + np.asarray(
    ...
    print(f"carrier body:             {mechanics.carrier_body}")
