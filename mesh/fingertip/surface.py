...
class PadMesh:
    ...
    def metadata(self) -> Mapping[str, Any]:
        return MappingProxyType({"condition": "reference"})

    @property
    def reference_mesh(self) -> PadMesh:
        return self

    @property
    def displacement(self) -> np.ndarray:
        displacement = np.zeros_like(self.coordinates)
    ...
    def deformed(
        ...
        *,
        ...
        """Return an immutable loaded view with this topology."""
        values = np.array(displacement, dtype=float, copy=True)
        if values.shape != self.coordinates.shape:
            raise InvalidPadMesh("displacement must have shape (N, 2)")
        if not np.all(np.isfinite(values)):
            raise InvalidPadMesh("displacement must contain finite values")
        loaded_coordinates = self.coordinates + values
        loaded_areas = _signed_double_areas(loaded_coordinates, self.triangles)
        if np.any(loaded_areas <= _area_tolerance(loaded_coordinates)):
            raise InvalidPadMesh(
                "displacement creates a degenerate or inverted triangle"
            )
        values.setflags(write=False)
        loaded_coordinates.setflags(write=False)
        return _DeformedPadMesh(
            self,
            values,
            loaded_coordinates,
...
@dataclass(frozen=True)
class _DeformedPadMesh:
    _reference: PadMesh
    _displacement: np.ndarray
    _coordinates: np.ndarray
    metadata: Mapping[str, Any]

    @property
    def node_ids(self) -> np.ndarray:
        return self._reference.node_ids
    ...
    def reference_coordinates_mm(self) -> np.ndarray:
        return self._reference.reference_coordinates_mm

    @property
    def triangles(self) -> np.ndarray:
        return self._reference.triangles

    @property
    def boundary_edges(self) -> np.ndarray:
        return self._reference.boundary_edges

    @property
    def boundaries(self) -> Mapping[str, np.ndarray]:
        return self._reference.boundaries

    @property
    def boundary_edges_by_tag(self) -> Mapping[str, np.ndarray]:
        return self._reference.boundaries

    @property
    def semantic_boundary_tags(self) -> tuple[str, ...]:
        return self._reference.semantic_boundary_tags
    ...
    def boundary_edges_for(self, tag: str) -> np.ndarray:
        return self._reference.boundary_edges_for(tag)

    def boundary_node_indices_for(self, tag: str) -> np.ndarray:
        return self._reference.boundary_node_indices_for(tag)
    ...
    def deformed(
        ...
        *,
        ...
        return self._reference.deformed(displacement, metadata=metadata)
